...
class Display(object):
    ...
    def __init__(self, spi, cs, dc, rst,
        ...
        Args:
            ...
            width (Optional int): Screen width (default 480)
...
#        print("> fill_vrect() : remainder = " + str(remainder) )
        if remainder:
...
#            buf = color666(rgb_list[0], rgb_list[1], rgb_list[2])
    ...
    def fill_vrect_rgb(self, x, y, w, h, rgb_list):
        """Draw a filled rectangle (optimized for vertical drawing).
        Args:
            x (int): Starting X position.
            y (int): Starting Y position.
            ...
            color (int): RGB565 color value.
        """
#        print("> fill_vrect_rgb() :" +" x="+str(x) +" y="+str(y) +" w="+str(w) +" h="+str(h) +" r="+str(rgb_list[0]) +" g="+str(rgb_list[1]) +" b="+str(rgb_list[2]) )
        if self.is_off_grid(x, y, x + w - 1, y + h - 1):
            return
        chunk_width = 1024 // h
        chunk_count, remainder = divmod(w, chunk_width)
...
#            buf = color.to_bytes(2, 'big') * chunk_size
#            buf = color666(rgb_list[0], rgb_list[1], rgb_list[2])
#            buf = color565(rgb_list[0], rgb_list[1], rgb_list[2]).to_bytes(2, 'big') * chunk_size 
#            buf = color565(rgb_list[0], rgb_list[1], rgb_list[2]).to_bytes(3, 'big') * chunk_size 
            buf = ( ((255 - rgb_list[0]) << 16) + ((255 - rgb_list[1]) << 8) + ((255 - rgb_list[2]) << 0) ).to_bytes(3, 'big') * chunk_size 
#            buf = color565( 0xFF , rgb_list[0], rgb_list[1], rgb_list[2] , 0xFF ).to_bytes(4, 'big') * chunk_size 
#            print("> fill_vrect_rgb() : buf = " + str(buf) )
            for c in range(0, chunk_count):
                self.block(chunk_x, y,
                           chunk_x + chunk_width - 1, y + h - 1,
                           buf)
...
#        print("> fill_vrect_rgb() : remainder = " + str(remainder) )       
        if remainder:
#            buf = color.to_bytes(2, 'big') * remainder * h
            buf = color666(rgb_list[0], rgb_list[1], rgb_list[2])
            self.block(chunk_x, y,
